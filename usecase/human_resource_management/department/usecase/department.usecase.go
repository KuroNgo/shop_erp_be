...
func (d departmentUseCase) CreateOne(ctx context.Context, department *departmentsdomain.Input) error {
...
func (d departmentUseCase) UpdateOne(ctx context.Context, id string, department *departmentsdomain.Input) error {
	ctx, cancel := context.WithTimeout(ctx, d.contextTimeout)
	defer cancel()

	err := d.departmentRepository.UpdateOne(ctx, id, department)
	if err != nil {
		return err
	}
